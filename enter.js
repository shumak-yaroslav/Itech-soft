  ...
  data() {
    ...
    methods:{
      ...
      changePsw(){
        ...
        let findObj = {};
        let val_1 = [];
        val_1.push(this.Username_psw);
        for (let i = 0; i <= key.length - 1; i++) {
          findObj[key[i]] = val_1[i];
        }
        console.log(findObj);

        const result = this.users.filter((some) =>
          Object.keys(findObj).every((key) => some[key] === findObj[key])
        );

        console.log(result);

        user = this.Username_psw;
      ...
      changePassword() {
        let findObj = {};
        let val_2 = [];
        let l4 = this.Password1
        let l5 = this.Password2
